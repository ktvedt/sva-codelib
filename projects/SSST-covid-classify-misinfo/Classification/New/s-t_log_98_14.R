...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg_98_filtered_nort/misinformation_class_13_98_nort.RDS")
...
train |>
  ungroup() |>
  count(label) # misinfo = 10888, non misinfo = 136531

################################################################################
147419/(table(train$label)[1] * 2) # 6.769792              
147419/(table(train$label)[2] * 2) # 0.5398737            

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 6.769792, 0.5398737),
...
cm_lr$byClass["F1"] # 0.9830149               
cm_lr$byClass["Precision"] # 0.9989213              
cm_lr$byClass["Recall"] # 0.9676071             
...
lr_preds_all_filtered_label |> # 98
  ungroup() |>
  count(label) # misinfo = 577, nonmisinfo = 6747
...
covid_predicted |> # 98
  ungroup() |>
  count(label) # misinfo = 14246, nonmisinfo = 177352

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg_98_filtered_nort/misinformation_class_14_98_nort.RDS")
