...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg_98_filtered_nort/misinformation_class_15_98_nort.RDS")
...
train |>
  ungroup() |>
  count(label) # misinfo = 11797, non misinfo = 145867

################################################################################
157664/(table(train$label)[1] * 2) # 6.682377                
157664/(table(train$label)[2] * 2) # 0.5404375              

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 6.682377, 0.5404375),
...
cm_lr$byClass["F1"] # 0.9833503                 
cm_lr$byClass["Precision"] # 0.9989644                
cm_lr$byClass["Recall"] # 0.9682168               
...
lr_preds_all_filtered_label |> # 98
  ungroup() |>
  count(label) # misinfo = 328, nonmisinfo = 2750
...
covid_predicted |> # 98
  ungroup() |>
  count(label) # misinfo = 15022, nonmisinfo = 185136

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg_98_filtered_nort/misinformation_class_16_98_nort.RDS")
