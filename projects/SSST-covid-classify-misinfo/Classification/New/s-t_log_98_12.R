...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg_98_filtered_nort/misinformation_class_11_98_nort.RDS")
...
train |>
  ungroup() |>
  count(label) # misinfo = 9546, non misinfo = 117270

################################################################################
126816/(table(train$label)[1] * 2) # 6.642363            
126816/(table(train$label)[2] * 2) # 0.5407009          

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 6.642363, 0.5407009),
...
cm_lr$byClass["F1"] # 0.9816459             
cm_lr$byClass["Precision"] # 0.9983942            
cm_lr$byClass["Recall"] # 0.9654503           
...
lr_preds_all_filtered_label |> # 98
  ungroup() |>
  count(label) # misinfo = 842, nonmisinfo = 13122
...
covid_predicted |> # 98
  ungroup() |>
  count(label) # misinfo = 12879, nonmisinfo = 159605

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg_98_filtered_nort/misinformation_class_12_98_nort.RDS")
