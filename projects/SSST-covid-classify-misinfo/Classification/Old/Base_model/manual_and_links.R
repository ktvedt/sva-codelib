...
covid_links <- readRDS("~/INORK/Data/covid_misinfo_links_only.RDS")

covid_links <- covid_links |>
  sample_n(735, seed = 321)
...
train |>
  count(label) # misinfo = 799, non misinfo = 799

# saveRDS(train, "~/INORK/Data/manual_links_TRAIN.RDS")
# train <- readRDS("~/INORK//Data/manual_links_TRAIN.RDS")

# saveRDS(test, "~/INORK/Data/manual_links_TEST.RDS")
# test <- readRDS("~/INORK/Data/manual_links_TEST.RDS")
...
cm_svm_test$byClass["F1"] # 0.2222222      
cm_svm_test$byClass["Precision"] # 0.3076923     
cm_svm_test$byClass["Recall"] # 0.173913     
...
svm_tune$best.parameters$cost # 2
...
cm_svm$byClass["F1"] # 0.1714286      
cm_svm$byClass["Precision"] # 0.2307692     
cm_svm$byClass["Recall"] # 0.1363636  
...
cm_lr$byClass["F1"] # 0.2325581       
cm_lr$byClass["Precision"] # 0.3846154      
cm_lr$byClass["Recall"] # 0.1666667   
...
cm_bt$byClass["F1"] # 0.2631579    
cm_bt$byClass["Precision"] # 0.3846154     
...
cm_rf$byClass["F1"] # 0.2439024     
cm_rf$byClass["Precision"] # 0.3846154 
cm_rf$byClass["Recall"] # 0.1785714 
