...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_2.RDS")
...
train |>
  count(label) # misinfo = 11880, non misinfo = 87569

################################################################################
99449/(table(train$label)[1] * 2) # 4.185564     
99449/(table(train$label)[2] * 2) # 0.5678322     

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 4.185564, 0.5678322),
...
cm_lr$byClass["F1"] # 0.9926421      
cm_lr$byClass["Precision"] # 0.9989902    
cm_lr$byClass["Recall"] # 0.9863742 

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "~/INORK_ST/Self_train/Log_reg/Classifier/round_3.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 16504, nonmisinfo = 120419
...
covid_predicted |>
  count(label) # misinfo = 17360, nonmisinfo = 126214

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_3.RDS")
