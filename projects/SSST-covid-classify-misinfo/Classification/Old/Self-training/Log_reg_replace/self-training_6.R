...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_5.RDS")
...
train |>
  count(label) # misinfo = 16392, non misinfo = 129344

################################################################################
145736/(table(train$label)[1] * 2) # 4.445339         
145736/(table(train$label)[2] * 2) # 0.5633659       

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 4.445339, 0.5633659),
...
cm_lr$byClass["F1"] # 0.9928424         
cm_lr$byClass["Precision"] # 0.9985359       
cm_lr$byClass["Recall"] # 0.9872135    

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "~/INORK_ST/Self_train/Log_reg/Classifier/round_6.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 20957, nonmisinfo = 170255
...
covid_predicted |>
  count(label) # misinfo = 21872, nonmisinfo = 176068

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_6.RDS")
