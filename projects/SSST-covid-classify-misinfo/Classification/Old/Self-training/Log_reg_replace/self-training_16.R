...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_15.RDS")
...
train |>
  count(label) # misinfo = 22278, non misinfo = 246523

################################################################################
268801/(table(train$label)[1] * 2) # 6.03288                   
268801/(table(train$label)[2] * 2) # 0.5451844                

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 6.03288, 0.5451844),
...
cm_lr$byClass["F1"] # 0.9919433               
cm_lr$byClass["Precision"] # 0.9989301                
cm_lr$byClass["Recall"] # 0.9850536            

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "E:/Data/Training samples/st_log_reg/Log_reg_classifier/round_16.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 26489, nonmisinfo = 303025
...
covid_predicted |>
  count(label) # misinfo = 28207, nonmisinfo = 313095

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_16.RDS")
