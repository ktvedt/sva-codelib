...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_6.RDS")
...
train |>
  count(label) # misinfo = 17497, non misinfo = 140854

################################################################################
158351/(table(train$label)[1] * 2) # 4.52509          
158351/(table(train$label)[2] * 2) # 0.5621104        

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 4.52509, 0.5621104),
...
cm_lr$byClass["F1"] # 0.9934106          
cm_lr$byClass["Precision"] # 0.9993143        
cm_lr$byClass["Recall"] # 0.9875762     

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "~/INORK_ST/Self_train/Log_reg/Classifier/round_7.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 21869, nonmisinfo = 184898
...
covid_predicted |>
  count(label) # misinfo = 22817, nonmisinfo = 191071

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_7.RDS")
