...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_8.RDS")
...
train |>
  count(label) # misinfo = 19050, non misinfo = 164375

################################################################################
183425/(table(train$label)[1] * 2) # 4.814304            
183425/(table(train$label)[2] * 2) # 0.5579468          

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 4.814304, 0.5579468),
...
cm_lr$byClass["F1"] # 0.9921769           
cm_lr$byClass["Precision"] # 0.9985303         
cm_lr$byClass["Recall"] # 0.9859038      

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "E:/Data/Training samples/st_log_reg/Log_reg_classifier/round_9.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 23361, nonmisinfo = 214906
...
covid_predicted |>
  count(label) # misinfo = 24488, nonmisinfo = 221892

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_9.RDS")
