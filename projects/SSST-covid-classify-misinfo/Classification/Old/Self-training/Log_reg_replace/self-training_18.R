...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_17.RDS")
...
train |>
  count(label) # misinfo = 22799, non misinfo = 252926

################################################################################
275725/(table(train$label)[1] * 2) # 6.046866                     
275725/(table(train$label)[2] * 2) # 0.5450705                  

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 6.046866, 0.5450705),
...
cm_lr$byClass["F1"] # 0.991499                 
cm_lr$byClass["Precision"] # 0.9993006                  
cm_lr$byClass["Recall"] # 0.9838182              
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 26898, nonmisinfo = 310460
...
covid_predicted |>
  count(label) # misinfo = 28661, nonmisinfo = 320538

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_18.RDS")
