...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_3.RDS")
...
train |>
  count(label) # misinfo = 13888, non misinfo = 100971

################################################################################
114859/(table(train$label)[1] * 2) # 4.135189       
114859/(table(train$label)[2] * 2) # 0.5687722     

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 4.135189, 0.5687722),
...
cm_lr$byClass["F1"] # 0.9919908       
cm_lr$byClass["Precision"] # 0.9988479     
cm_lr$byClass["Recall"] # 0.9852273  

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "~/INORK_ST/Self_train/Log_reg/Classifier/round_4.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 18291, nonmisinfo = 137642
...
covid_predicted |>
  count(label) # misinfo = 19160, nonmisinfo = 143398

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_4.RDS")
