...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_7.RDS")
...
train |>
  count(label) # misinfo = 18253, non misinfo = 152856

################################################################################
171109/(table(train$label)[1] * 2) # 4.687147           
171109/(table(train$label)[2] * 2) # 0.5597065         

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 4.687147, 0.5597065),
...
cm_lr$byClass["F1"] # 0.9921688          
cm_lr$byClass["Precision"] # 0.9993427        
cm_lr$byClass["Recall"] # 0.9850972     

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "~/INORK_ST/Self_train/Log_reg/Classifier/round_8.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # 22790 = 20957, nonmisinfo = 198636
...
covid_predicted |>
  count(label) # misinfo = 23813, nonmisinfo = 205469

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_8.RDS")
