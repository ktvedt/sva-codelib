...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_11.RDS")
...
train |>
  count(label) # misinfo = 20765, non misinfo = 208190

################################################################################
228955/(table(train$label)[1] * 2) # 5.513003               
228955/(table(train$label)[2] * 2) # 0.5498703            

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.513003, 0.5498703),
...
cm_lr$byClass["F1"] # 0.9923965           
cm_lr$byClass["Precision"] # 0.9986775            
cm_lr$byClass["Recall"] # 0.986194        

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "E:/Data/Training samples/st_log_reg/Log_reg_classifier/round_12.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 25092, nonmisinfo = 266655
...
covid_predicted |>
  count(label) # misinfo = 26564, nonmisinfo = 275207

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_12.RDS")
