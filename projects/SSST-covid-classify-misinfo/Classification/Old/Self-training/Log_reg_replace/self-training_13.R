...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_12.RDS")
...
train |>
  count(label) # misinfo = 21294, non misinfo = 220122

################################################################################
241416/(table(train$label)[1] * 2) # 5.668639                
241416/(table(train$label)[2] * 2) # 0.5483686             

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.668639, 0.5483686),
...
cm_lr$byClass["F1"] # 0.9927406            
cm_lr$byClass["Precision"] # 0.9990512             
cm_lr$byClass["Recall"] # 0.9865093         

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "E:/Data/Training samples/st_log_reg/Log_reg_classifier/round_13.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 25590, nonmisinfo = 279464
...
covid_predicted |>
  count(label) # misinfo = 27095, nonmisinfo = 288320

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_13.RDS")
