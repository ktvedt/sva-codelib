...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_14.RDS")
...
train |>
  count(label) # misinfo = 22007, non misinfo = 239057

################################################################################
261064/(table(train$label)[1] * 2) # 5.931385                  
261064/(table(train$label)[2] * 2) # 0.5460288               

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.931385, 0.5460288),
...
cm_lr$byClass["F1"] # 0.9921784              
cm_lr$byClass["Precision"] # 0.9985523               
cm_lr$byClass["Recall"] # 0.9858853           

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "E:/Data/Training samples/st_log_reg/Log_reg_classifier/round_15.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 26261, nonmisinfo = 298826
...
covid_predicted |>
  count(label) # misinfo = 27886, nonmisinfo = 308116

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_15.RDS")
