...
train |>
  count(label) # misinfo = 5053, non misinfo = 51 891

################################################################################
56944/(table(train$label)[1] * 2) # 5.634672    
56944/(table(train$label)[2] * 2) # 0.5486886    

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.634672, 0.5486886),
...
cm_lr$byClass["F1"] # 0.9923858      
cm_lr$byClass["Precision"] # 0.9991482  
cm_lr$byClass["Recall"] # 0.9857143 

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "~/INORK/Self_train/Log_reg/Classifier/round_2.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 13954, nonmisinfo = 103428
...
covid_predicted |>
  count(label) # misinfo = 14851, nonmisinfo = 109462

saveRDS(covid_predicted, "~/INORK/Self_train/Log_reg/Results/misinformation_class_2.RDS")
