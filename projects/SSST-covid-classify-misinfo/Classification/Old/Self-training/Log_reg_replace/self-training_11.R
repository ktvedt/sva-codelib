...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_10.RDS")
...
train |>
  count(label) # misinfo = 20138, non misinfo = 194874

################################################################################
215012/(table(train$label)[1] * 2) # 5.338465              
215012/(table(train$label)[2] * 2) # 0.5516693           

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.338465, 0.5516693),
...
cm_lr$byClass["F1"] # 0.9916326           
cm_lr$byClass["Precision"] # 0.9986283           
cm_lr$byClass["Recall"] # 0.9847343        

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "E:/Data/Training samples/st_log_reg/Log_reg_classifier/round_11.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 24677, nonmisinfo = 252083
...
covid_predicted |>
  count(label) # misinfo = 26058, nonmisinfo = 260136

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_11.RDS")
