...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_16.RDS")
...
train |>
  count(label) # misinfo = 22599, non misinfo = 250442

################################################################################
273041/(table(train$label)[1] * 2) # 6.040997                    
273041/(table(train$label)[2] * 2) # 0.5451182                 

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 6.040997, 0.5451182),
...
cm_lr$byClass["F1"] # 0.9911567                
cm_lr$byClass["Precision"] # 0.9992867                 
cm_lr$byClass["Recall"] # 0.9831579             

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "E:/Data/Training samples/st_log_reg/Log_reg_classifier/round_17.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 26900, nonmisinfo = 305593
...
covid_predicted |>
  count(label) # misinfo = 28518, nonmisinfo = 316139

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_17.RDS")
