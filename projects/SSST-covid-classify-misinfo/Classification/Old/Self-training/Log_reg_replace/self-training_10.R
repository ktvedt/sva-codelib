...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg/misinformation_class_9.RDS")
...
train |>
  count(label) # misinfo = 19580, non misinfo = 177524

################################################################################
197104/(table(train$label)[1] * 2) # 5.033299             
197104/(table(train$label)[2] * 2) # 0.5551475          

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.033299, 0.5551475),
...
cm_lr$byClass["F1"] # 0.9927052          
cm_lr$byClass["Precision"] # 0.9981663          
cm_lr$byClass["Recall"] # 0.9873035       

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

saveRDS(lr_final_fit, "E:/Data/Training samples/st_log_reg/Log_reg_classifier/round_10.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 23950, nonmisinfo = 236158
...
covid_predicted |>
  count(label) # misinfo = 25241, nonmisinfo = 243525

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg/misinformation_class_10.RDS")
